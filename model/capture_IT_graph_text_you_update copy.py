...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        #         txt_embedding, txt_attention_mask, entity_embedding, entity_attention_mask, co_IPV_attention_mask,
        ...
        use_co_attention_mask = False
        count = 0
        for t_layer_id1,t_layer_id2 in zip(self.t_biattention_id,self.t_biattention_id):
            # do the bi attention.
            txt_embedding, entity_embedding, co_attention_probs = self.c_layer[count](
                txt_embedding, txt_attention_mask, entity_embedding, entity_attention_mask, co_TPV_attention_mask,
                use_co_attention_mask)

            # use_co_attention_mask = False
            if output_all_attention_masks:
                all_attention_TPV_mask_c.append(co_attention_probs)
            count += 1
...
class BertEntityFc(nn.Module):
    def __init__(self, config):
        super(BertEntityFc, self).__init__()
        self.dense = nn.Linear(config.hidden_size*2, config.bi_hidden_size)
        self.activation = nn.ReLU()
...
class BertModel(BertPreTrainedModel):
    def __init__(self, config):
        ...
        #self.entity_fc = nn.Linear(config.bi_hidden_size, config.hidden_size)
    ...
    def forward(
        ...
        if co_TPV_attention_mask is None:
            co_TPV_attention_mask = torch.zeros(input_txt.size(0), input_entity.size(1), input_txt.size(1)).type_as(extended_entity_attention_mask)
        ...
        extended_co_TPV_attention_mask = co_TPV_attention_mask.unsqueeze(1)
        extended_co_TPV_attention_mask = extended_co_TPV_attention_mask * 5.0
        extended_co_TPV_attention_mask = extended_co_TPV_attention_mask.to(dtype=torch.float32)
        ...
        for batch_i in range(each_entity_ids.shape[0]):
            ...
            each_entity_output_t[batch_i,:,:] =  F.normalize(text_out, dim=-1) #encoded_each_entity_layers[-1] #[batch, entity_num, 36, dim]
        ...
        CLR_loss, all_attention_mask = self.encoder(
            ...
            extended_co_TPV_attention_mask,
        ...
        # co encoder
        sequence_output_t1, sequence_output_v = self._get_cross_output(
        ...
        # )
        sequence_output_entity, sequence_output_t2 = self._get_cross_output(
            sequence_output_entity,sequence_output_t,
            entity_mask, attention_mask
        )
        
        #sequence_output_v =  torch.cat((sequence_output_v_1, sequence_output_v_2), dim=2)
        sequence_output_t =  torch.cat((sequence_output_t1, sequence_output_t2), dim=2)
        ...
        sequence_output_t=self.entity_fc(sequence_output_t)
        ...
        sequence_output_v = F.normalize(sequence_output_v, dim=-1)
        sequence_output_t = F.normalize(sequence_output_t, dim=-1)
        sequence_output_entity = F.normalize(sequence_output_entity, dim=-1)
        pooled_output_t = F.normalize(pooled_output_t, dim=-1)
        pooled_output_v = F.normalize(pooled_output_v, dim=-1)
        pooled_output_entity = F.normalize(pooled_output_entity, dim=-1)
...
class BertForMultiModalPreTraining(BertPreTrainedModel):
    ...
    def forward(
        ...
        if(entity_sim is None):
            ...
            #pdb.set_trace()
            sorted_batch_entity_sim, index =  torch.sort(batch_entity_sim, descending=True)
            ...
            entity_fea_matrix = entity_fea_matrix.cuda()
            graph_num = 1
            graph_node_num = 5
            entity_graph_contras_loss = 0
            pos_threshold = 0.85
            neg_threshold = 0.5
            pos_graph_node_num = 3
            neg_graph_node_num = 3
            for entity_i in range(len(batch_index)):
                # node loss
                pos_index = (sorted_batch_entity_sim[entity_i,:]>pos_threshold).nonzero(as_tuple=False)
                neg_index = (sorted_batch_entity_sim[entity_i,:]<neg_threshold).nonzero(as_tuple=False)
                tmp_loss= entity_loss(entity_fea_matrix[entity_i,:], entity_fea_matrix[index[entity_i,torch.randperm(len(neg_index))[0]].cpu(),:] , torch.tensor([1]).cuda()) #index[entity_i,pos_index[torch.randperm(len(pos_index))[0]]].cpu()
                entity_contras_loss += tmp_loss
                # positive_sample = entity_fea_matrix[index[entity_i,pos_index].cpu(),:]
                # negative_sample = entity_fea_matrix[index[entity_i,neg_index].cpu(),:]
                # for pos_index_i in pos_index:
                #     for neg_index_i in neg_index:
                #         tmp_loss= entity_loss(entity_fea_matrix[index[entity_i,pos_index_i].cpu(),:], entity_fea_matrix[index[entity_i,neg_index_i].cpu(),:] , torch.tensor([1]).cuda())
                #         entity_contras_loss += tmp_loss
                # subgraph loss
                # positive and negative graph
                # if(len(pos_index)>5):
                #     pos_graph_node_num = 5
                # else:
                #     pos_graph_node_num = 3#int(len(pos_index)/2)

                # if(len(neg_index)>5):
                #     neg_graph_node_num = 5
                # else:
                #     neg_graph_node_num = 3 #int(len(neg_index)/2)

                # pos_select_node = entity_fea_matrix[index[entity_i, pos_index[torch.randperm(len(pos_index))[0:pos_graph_node_num]]].cpu(),:].cuda()
                ...
                # if(len(pos_global_add_pool)<neg_graph_node_num):
                #     neg_graph_node_num = len(pos_global_add_pool)
    
                # neg_select_node = entity_fea_matrix[index[entity_i, neg_index[torch.randperm(len(neg_index))[0:neg_graph_node_num]]].cpu(),:].cuda()
                ...
                # tmp_loss = entity_loss(pos_global_add_pool, neg_global_add_pool,  torch.tensor([1]).cuda())
        ...
        if return_features:
            ...
            return masked_lm_loss.unsqueeze(0).to(torch.float32), \
                   ...
                   entity_contras_loss.unsqueeze(0).to(torch.float32), \
                   CLR_loss.unsqueeze(0).to(torch.float32), \
