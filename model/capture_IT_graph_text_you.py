...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        for batch_i in range(each_entity_ids.shape[0]):
            ...
            each_entity_output_t[batch_i,:,:] = text_out # F.normalize(text_out, dim=-1) #encoded_each_entity_layers[-1] #[batch, entity_num, 36, dim]
    ...
    def forward(
        ...
        if(entity_sim is None):
            ...
            for entity_i in range(len(batch_index)):
                ...
                tmp_loss= entity_loss(entity_fea_matrix[index[entity_i,pos_index[torch.randperm(len(pos_index))[0]]].cpu(),:], entity_fea_matrix[index[entity_i,torch.randperm(len(neg_index))[0]].cpu(),:] , torch.tensor([1]).cuda())
