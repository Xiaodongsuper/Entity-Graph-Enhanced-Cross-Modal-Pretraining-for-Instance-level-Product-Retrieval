...
from model.capture_IT_no_graph_text_you import BertForMultiModalPreTraining, BertConfig
    ...
    for epochId in range(int(args.start_epoch), int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(tqdm(train_dataset)):
            ...
            if step % 100 == 0 and step != 0:
                masked_loss_t_tmp = masked_loss_t_tmp / 100.0
                masked_loss_v_tmp = masked_loss_v_tmp / 100.0
                masked_loss_pv_tmp = masked_loss_pv_tmp/100.0
                next_sentence_loss_tmp = next_sentence_loss_tmp / 100.0
                loss_tmp = loss_tmp / 100.0
                ...
                printFormat = "[%s][Ep: %.2f][Iter: %d][Time: %5.2fs][Loss: %.5g][Loss_v: %.5g][Loss_t: %.5g][Loss_pv: %.5g][Loss_n: %.5g][LR: %.8g][epoch: %d][step: %d]"
