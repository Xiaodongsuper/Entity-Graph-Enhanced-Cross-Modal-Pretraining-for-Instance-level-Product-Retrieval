...
from model.capture_IT_graph_text_you_update import BertForMultiModalPreTraining, BertConfig
    ...
    for epochId in range(int(args.start_epoch), int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(tqdm(train_dataset)):
            ...
            if step % 2 == 0 and step != 0:
                masked_loss_t_tmp = masked_loss_t_tmp / 2.0
                masked_loss_v_tmp = masked_loss_v_tmp / 2.0
                entity_contras_loss_tmp = entity_contras_loss_tmp /2.0
                
                next_sentence_loss_tmp = next_sentence_loss_tmp / 2.0
                loss_tmp = loss_tmp / 2.0
                ...
                printFormat = "[%s][Ep: %.2f][Iter: %d][Time: %5.2fs][Loss: %.5g][Loss_v: %.5g][Loss_t: %.5g][Loss_pv: %.5g][Loss_node: %.5g][Loss_n: %.5g][LR: %.8g][epoch: %d][step: %d]"
        ...
        entity_node_fea /=  entity_node_fea.sum(axis=1)[:,np.newaxis]*10
