...
from model.capture_IT_graph import BertForMultiModalPreTraining, BertConfig
import torch.nn.functional as F
    ...
    for epochId in range(int(args.start_epoch), int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(tqdm(train_dataset)):
            ...
            if step % 100 == 0 and step != 0:
                ...
                printFormat = "[%s][Ep: %.2f][Iter: %d][Time: %5.2fs][Loss: %.5g][Loss_v: %.5g][Loss_t: %.5g][Loss_node: %.5g][Loss_graph: %.5g][Loss_n: %.5g][LR: %.8g][epoch: %d][step: %d]"
        ...
        update_entity_graph_fea = F.cosine_similarity(entity_node_fea, entity_node_fea, 0)
