...
for rs_i in matched_rs.readlines():
    ...
    shutil.copy('/data1/xl/multi_modal/Retrieval/data/images/'+retrieval_rs[0]+'.jpg', './several_mathed_rs/'+retrieval_rs[0]+'/!'+retrieval_rs[0]+'.jpg')
    tmp = 1
    for img_i in retrieval_rs[1:]:
        tmp_img_num = 0
        shutil.copy('/data1/xl/multi_modal/Retrieval/data/images/'+img_i+'.jpg', './several_mathed_rs/'+retrieval_rs[0]+'/'+str(tmp)+'.jpg')
        tmp +=1
        if(tmp>20):
            break
    rs_num +=1 
